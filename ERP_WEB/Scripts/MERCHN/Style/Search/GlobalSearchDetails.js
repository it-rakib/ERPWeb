...
var GlobalSearchDetailsHelper = {
    Init() {
        GlobalSearchDetailsHelper.ShowSummary();
        $("#pnlMaster").kendoPanelBar({
            expandMode: "multiple",
            expanded: true
        });
        StyleInfoHelper.Init();
        GlobalSearchDetailsHelper.InitCombos();
        SSCIHelper.InitUploadEvents();
        GlobalSearchDetailsHelper.InitBtn();
        SearchSummaryHelper.InitSearchSummary();
    },
    InitBtn() {
        $("#btnApprove").click(function () {
           // var statusId = $("#cboStatus").data("kendoComboBox").value();
            ...
            //
            var obj = GlobalSearchDetailsHelper.GetStyleARObj("720B6821-33A4-4B97-982A-8CC8B787513D");
                        ...
                        AjaxManager.MsgBox('success', 'center', 'Success Message', jsonData.Message,
                            [{
                                addClass: 'btn btn-primary', text: 'Ok', onClick: function ($noty) {
                                    $noty.close();
                                    GlobalSearchDetailsHelper.ResetForm();
                                    GlobalSearchDetailsHelper.ShowSummary();
                                }
                            }]);
                    } else {
        ...
        $("#btnReject").click(function () {
            ...
            //var parentId = $("#cboStyleSearch").data("kendoMultiColumnComboBox").value();
            //
            var obj = GlobalSearchDetailsHelper.GetStyleARObj("3CEB6E53-4CF8-4C46-A0B0-03195CF1C34A");
            var jsonParam = JSON.stringify(obj);
            var serviceUrl = _baseUrl + "/api/Styles/CreateAcknowledgement";
            AjaxManager.PostJsonApi(serviceUrl, jsonParam, onSuccess, onFailed);
                function onSuccess(jsonData) {
    ...
    GetStyleObj() {
        ...
        if (documents.length > 0) {
            for (var obj1 of documents) {
                if (obj1["DocFile"] != null) {
                    model.append("Documents[" + index + "].DocId", obj1["DocId"]);
                    model.append("Documents[" + index + "].DocTypeId", obj1["DocTypeId"]);
                    model.append("Documents[" + index + "].DocFile", obj1["DocFile"]);
                    model.append("Documents[" + index + "].StyleDetailId", obj1["StyleDetailId"]);
                    index++;
                }
                else {
                    model.append("Documents[" + index + "].DocId", obj1["DocId"]);
                    model.append("Documents[" + index + "].DocTypeId", obj1["DocTypeId"]);
                    model.append("Documents[" + index + "].DocFilenameWithPath", obj1["DocFilenameWithPath"]);
                    model.append("Documents[" + index + "].StyleDetailId", obj1["StyleDetailId"]);
                    index++;
                }
            }
        ...
        var styleDetails = [
            {
                ...
                StyleColors: GlobalSearchDetailsHelper.GetColors(),
                StyleSizes: GlobalSearchDetailsHelper.GetSizes()
            }
        ];
        index = 0;
        var cIndex = 0,
            sIndex = 0;
        for (var obj of styleDetails) {
    ...
    InitCombos() {
        ...
        $("#cboColor").kendoComboBox({
            ...
            change: function () {
                ...
                if (!UtilityHelper.IsNullOrEmpty(selectedItem.ColorId)) GlobalSearchDetailsHelper.AddColor(selectedItem);
                GlobalSearchDetailsHelper.ResetColor();
            }
        });
        ...
        $("#cboSize").kendoComboBox({
            ...
            change: function () {
                ...
                if (!UtilityHelper.IsNullOrEmpty(selectedItem.SizeId)) GlobalSearchDetailsHelper.AddSize(selectedItem);
                GlobalSearchDetailsHelper.ResetSize();
            }
        });
        ...
        $("#cboStyleSearch").kendoMultiColumnComboBox({
            ...
            change: function () {
                ...
                if ($.trim(buyerId).length == 0 && $.trim(brandId).length == 0) {
                    ...
                    GlobalSearchDetailsHelper.FillStyleForm(style, false);
                }
            }
        });

        $("#btnSearchStyle").click(function () {
            GlobalSearchDetailsHelper.LoadStyle();
        });
    ...
    LoadStyle() {
        GlobalSearchDetailsHelper.ResetStyleSearch();
    ...
    FillStyleForm: function (obj, isEdit) {
        ...
        if (styleDetail != null) {
            var colors = styleDetail.StyleColors;
            $.map(colors, function (color) {
                GlobalSearchDetailsHelper.AddColor(color);
            });
            GlobalSearchDetailsHelper.ResetColor();

            var sizes = styleDetail.StyleSizes;
            $.map(sizes, function (size) {
                GlobalSearchDetailsHelper.AddSize(size);
            });
            GlobalSearchDetailsHelper.ResetSize();
            ...
            $.map(docs, function (doc) {
                if (!isUndefined) {
                    ...
                    else if (doc.DocTypeId == _files.FileTypeId) {
    ...
    ResetForm() {
        ...
        $("#gridSearchSummary").data("kendoGrid").dataSource.read();
    },
    ShowSummary() {
        $("#divSearchDetails").hide();
        $("#divSearchSummary").show();
    },
    ShowDetail() {
        $("#divSearchDetails").show();
...
var StyleInfoHelper = {
    ...
    ChangeEventOfCategoryCombo: function () {
        ...
        var data = GlobalSearchDetailsHelper.GetProductByCategoryId(productCategoryId);
