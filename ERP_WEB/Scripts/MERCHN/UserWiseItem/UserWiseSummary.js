var UserWiseItemSummaryManager = {
...
var UserWiseItemSummaryHelper = {
    InitUserWiseItemSummary: function () {
        UserWiseItemSummaryHelper.GenerateUserGrid();
        $("#cmbUser").change(function () {
            UserWiseItemSummaryHelper.LoadUserGrid();
        });
        $("#lblSearchUser").click(function () {
            UserWiseItemSummaryHelper.LoadUserCombo();
        });
    ...
    LoadUserGrid:function() {
        ...
        var data = UserWiseItemSummaryManager.gridDataSource(usrId);
        $("#grdUserSummary").data("kendoGrid").setDataSource(data);
    },

    GenerateUserGrid: function () {
        $("#grdUserSummary").kendoGrid({
            dataSource: UserWiseItemSummaryManager.gridDataSource(0),
            pageable: {
                refresh: true,
                serverPaging: true,
                serverFiltering: true,
            ...
            filterable: true,
            sortable: true,
            columns: UserWiseItemSummaryHelper.GenerateUserColumns(),
            editable: false,
            navigatable: true,
            selectable: "row",
        });
        ...
        return columns = [
                ...
                { field: "USRDESIG", title: "Designation", width: 100 },
               {
                   field: "Action", title: "Action", filterable: false, width: 60, command: [{
                       name: "edit", text: "", iconClass: "k-icon k-i-search", className: "k-success", click: UserWiseItemSummaryHelper.clickEventForEditButton
                   }], sortable: false
               }
        ];
    },
