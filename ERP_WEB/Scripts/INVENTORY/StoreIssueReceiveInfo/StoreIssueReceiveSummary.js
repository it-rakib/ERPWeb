var StoreIssueReceiveSummaryManager = {
    ...
    IssueReceiveGridDataSource: function (userId) {
        var gridDataSource = new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: _baseUrl + '/api/StoreIssueReceive/GetAllStoreIssueReceiveInfoGrid/' + userId,
                    type: "POST",
                    dataType: "json",
                    contentType: "application/json;",
                    cache: false,
...
var StoreIssueReceiveSummaryHelper = {
    InitStoreIssueReceiveSummary: function () {
        StoreIssueReceiveSummaryHelper.GenerateIssuePendingReceiveGrid();
        StoreIssueReceiveSummaryHelper.LoadPendingIssueReceiveGridDataSource();
        StoreIssueReceiveSummaryHelper.GenerateStoreIssueReceiveGrid();
        StoreIssueReceiveSummaryHelper.LoadIssueReceiveGridDataSource();
    },
    GenerateIssuePendingReceiveGrid: function () {
        $("#grdPendingIssueList").kendoGrid({
            dataSource: [],
            pageable: {
                refresh: true,
                serverPaging: true,
            ...
            columns: [
                ...
                {
                    command: [{
                        name: "edit", text: "Acknowledge", iconClass: "k-icon k-i-check-circle", className: "k-success", click: StoreIssueReceiveSummaryHelper.ClickEventForReceive
                    }], title: "Action &nbsp;"
                }
            ],
            editable: false,
    ...
    ClickEventForReceive: function (e) {
        e.preventDefault();
        var grid = $("#grdPendingIssueList").data("kendoGrid");
    ...
    LoadPendingIssueReceiveGridDataSource: function () {
        ...
        var grid = $("#grdPendingIssueList").data("kendoGrid");
        var data = StoreIssueReceiveSummaryManager.PendingIssueReceiveGridDataSource(CurrentUser.USERID);
        grid.setDataSource(data);
    },

    GenerateStoreIssueReceiveGrid: function () {
        $("#grdIssueReveiveSummary").kendoGrid({
            dataSource: [],
            pageable: {
                refresh: true,
                serverPaging: true,
            ...
            columns: [
                ...
                {
                    command: [{
                        name: "edit", text: "View", iconClass: "k-icon k-i-eye", className: "k-success", click: StoreIssueReceiveSummaryHelper.ClickEventForEdit
                    }], title: "Action &nbsp;"
                }
            ],
            editable: false,
    ...
    ClickEventForEdit: function (e) {
        e.preventDefault();
        var grid = $("#grdIssueReveiveSummary").data("kendoGrid");
        var tr = $(e.currentTarget).closest("tr");
        var selectedItem = this.dataItem(tr);
        grid.select(tr);
        if (selectedItem != null) {
            ...
            StoreIssueReceiveDetailsHelper.FillStoreIssueReceiveMasterForm(selectedItem);
        }
    },
    LoadIssueReceiveGridDataSource: function () {
        var grid = $("#grdIssueReveiveSummary").data("kendoGrid");
        var data = StoreIssueReceiveSummaryManager.IssueReceiveGridDataSource(CurrentUser.USERID);
        grid.setDataSource(data);
    }
};