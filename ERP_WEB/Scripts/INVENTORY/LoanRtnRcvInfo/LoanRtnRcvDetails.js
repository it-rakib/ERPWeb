var LoanRtnRcvDetailsManager = {
    SaveLoanRtnRcv: function () {
        var validator = $("#divDetails").kendoValidator().data("kendoValidator"),status = $(".status");
        if (validator.validate()) {
            if (LoanRtnRcvDetailsHelper.ValidateIssueForm()) {
                var issueObj = LoanRtnRcvDetailsHelper.CreateLoanRtnRcvObject();
                var jsonParam = JSON.stringify(issueObj);
                if (issueObj.LoanRtnRcvDetails != null && issueObj.LoanRtnRcvDetails.length > 0) {
                    var serviceUrl = _baseUrl + "/api/LoanRtnRcv/CreateOrUpdateLoanRtnRcv";
                ...
                AjaxManager.MsgBox('success', 'center', 'Success:', jsonData.Message,
                    [{
                        addClass: 'btn btn-primary', text: 'Ok', onClick: function ($noty) {
                            ...
                            $("#grdLoanRtnRcvItemSummary").data("kendoGrid").dataSource.read();
                            LoanRtnRcvDetailsHelper.FillLoanRtnRcvMasterForm(res);
                        }
                    }]);
            }
            else {
...
var LoanRtnRcvDetailsHelper = {
    InitLoanRtnRcvDetails: function () {
        $("#pnlMasterInfo").kendoPanelBar({
            expandMode: "multiple",
            expanded: true
        });
        RtnRcvItemListHelper.InitRtnRcvItemList();
        LoanRtnRcvDetailsHelper.GenerateDatePicker();
        ...
        $("#btnSave").click(function () {
            LoanRtnRcvDetailsManager.SaveLoanRtnRcv();
        });
    ...
    CreateLoanRtnRcvObject: function () {
        var obj = new Object();
        ...
        obj.LoanRtnRcvDetails = LoanRtnRcvDetailsHelper.CreateLoanRtnRcvDetailsObject();
        return obj;
    },
    CreateLoanRtnRcvDetailsObject: function () {
        var list = [];
        var itemGrid = $("#grdLoanRtnRcvItemSummary").data("kendoGrid");
    ...
    FillReturnMasterForm: function (obj) {
        ...
        LoanRtnRcvDetailsHelper.FillReturnDetailsGrid(obj);
    },
    FillReturnDetailsGrid: function (obj) {
        var itemGrid = $("#grdLoanRtnRcvItemSummary").data("kendoGrid");
        var list = RtnRcvItemListManager.GetLoanRtnDetailsItemByIssueId(obj.IssueId);
    ...
    FillLoanRtnRcvMasterForm: function (obj) {
        $("#btnSave").text(" Update");
        ...
        $("#hdnRequisitionId").val(obj.RequisitionId);
        $("#txtSrcRequisition").val(obj.RequisitionNo);
        LoanRtnRcvDetailsHelper.FillLoanRtnRcvDetailsGrid(obj);
    },
    FillLoanRtnRcvDetailsGrid: function (obj) {
        var itemGrid = $("#grdLoanRtnRcvItemSummary").data("kendoGrid");
        var receiveDetailsList = RtnRcvItemListManager.GetLoanRtnRcvDetailsByReceiveId(obj.ReceiveId);
        var gridDataSource = new kendo.data.DataSource({
    ...
    ClearAll: function () {
        ...
        $("#txtRemarks").val("");
        $("#grdLoanRtnRcvItemSummary").data("kendoGrid").dataSource.data([]);
    },
