var StyleListSearchManager = {
    GetStyleByTransTypeAndStoreIdDataSource: function (transTypeId,storeId) {
        var gridDataSource = new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: _baseUrl + "/api/SubContractReturn/GetStyleByTransTypeAndStoreId/" + $.trim(transTypeId) + "/" + $.trim(storeId),
                    type: "POST",
                    dataType: "json",
...
var StyleListSearchHelper = {
        ...
        $("#grdStyleSummary").on("dblclick", "tr.k-state-selected", function () {
            ...
            if (grid.select().length > 0) {
                ...
                if (selectedItem != null) {
                    SubContractReturnItemListHelper.LoadReturnItemListGridByStyle(selectedItem);
                    $("#popupStyleSearch").data("kendoWindow").close();
                }
            }
        });
    ...
    LoadStyleDataSource: function () {
        ...
        var grid = $("#grdStyleSummary").data("kendoGrid");
        var data = StyleListSearchManager.GetStyleByTransTypeAndStoreIdDataSource(transTypeId, storeId);
