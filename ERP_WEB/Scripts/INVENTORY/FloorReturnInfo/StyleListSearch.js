var StyleListSearchManager = {
    GetStyleGridByStoreIdDataSource: function (storeId) {
        var gridDataSource = new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: _baseUrl + '/api/FloorReturn/GetStyleGridByStoreId/' + $.trim(storeId),
                    type: "POST",
                    dataType: "json",
...
var StyleListSearchHelper = {
    InitStyleListSearch: function () {
        ...
        $('#txtsearch').on('input', function (e) {
            StyleListSearchHelper.LoadStyleDataSource();
        });
        ...
        $("#grdStyleSummary").on("dblclick", "tr.k-state-selected", function () {
            ...
            if (grid.select().length > 0) {
                ...
                if (selectedItem != null) {
                    FloorReturnItemListHelper.LoadReturnItemListGridByIssueId(selectedItem);
                    $("#popupStyleSearch").data("kendoWindow").close();
                }
            }
        });
    },
    LoadStyleDataSource: function () {
        var search = $("#txtsearch").val();
        var storeId = $("#cmbToStore").data("kendoComboBox").value();
        var grid = $("#grdStyleSummary").data("kendoGrid");
        var data = StyleListSearchManager.GetStyleGridByStoreIdDataSource(storeId);
    ...
    ClickEventForSelectButton: function (e) {
        ...
        grid.select(tr);
        if (selectedItem != null) {
            FloorReturnItemListHelper.LoadReturnItemListGridByStyle(selectedItem);
            $("#popupStyleSearch").data("kendoWindow").close();  
        }
    }
};