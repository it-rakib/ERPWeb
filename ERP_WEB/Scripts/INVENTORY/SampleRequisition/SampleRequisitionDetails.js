var SampleRequisitionDetailsManager = {
    SaveSampleRequisition: function () {
        var validator = $("#divDetails").kendoValidator().data("kendoValidator"), status = $(".status");
        if (validator.validate()) {
            if (SampleRequisitionDetailsHelper.ValidateRequisitionForm()) {                
                var requisitionObj = SampleRequisitionDetailsHelper.CreateSampleRequisitionObject();
                var jsonParam = JSON.stringify(requisitionObj);
                if (requisitionObj.SampleRequisitionDetails != null && requisitionObj.SampleRequisitionDetails.length > 0) {
                    var serviceUrl = _baseUrl + "/api/SampleRequisition/CreateOrUpdateSampleRequisition";
                ...
                AjaxManager.MsgBox('success', 'center', 'Success:', jsonData.Message,
                    [{
                        addClass: 'btn btn-primary', text: 'Ok', onClick: function ($noty) {
                            ...
                            $("#grdSampleRequisitionSummary").data("kendoGrid").dataSource.read();
                            SampleRequisitionDetailsHelper.FillRequisitionDetailsGrid(res);
                        }
                    }]);
            }
            else {
...
var SampleRequisitionDetailsHelper = {
    InitSampleRequisitionDetails: function () {
        RequisitionItemListHelper.InitRequisitionItemList();
        SampleRequisitionDetailsHelper.GenerateDatePicker();
        //SampleRequisitionDetailsHelper.GenerateNumericTextBox();
        ...
        $("#btnAddNew").click(function () {
            SampleRequisitionDetailsHelper.ClearAll();
            $("#divSummary").hide();
            $("#divDetails").show();
            $("#btnSave").text(" Save");
        ...
        $("#btnSave").click(function () {
            SampleRequisitionDetailsManager.SaveSampleRequisition();
        });
    ...
    CreateSampleRequisitionObject: function () {
        var obj = new Object();
        ...
        obj.SampleRequisitionDetails = SampleRequisitionDetailsHelper.CreateRequisitionDetailsObject();
        return obj;
    },
    ...
    FillRequisitionMasterForm: function (obj) {
        ...
        SampleRequisitionDetailsHelper.FillRequisitionDetailsGrid(obj);
    },
    FillRequisitionDetailsGrid: function (obj) {
        var detailsList = RequisitionItemListManager.GetSampleReqDetailsByRequisitionId(obj.RequisitionId);
        var itemGrid = $("#grdRequisitionItemSummary").data("kendoGrid");
