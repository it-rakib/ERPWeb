var IssueReceiveItemListManager = {
    GetFloorReceiveDetailsByReceiveId: function (receiveId) {
        var objItem = "";
        var jsonParam = "";
        var serviceUrl = _baseUrl + "/api/FloorIssueReceive/GetFloorReceiveDetailsByReceiveId/" + receiveId;
    ...
    GetFloorIssueDetailsItemByIssueId: function (issueId) {
        var objItem = "";
        var jsonParam = "";
        var serviceUrl = _baseUrl + "/api/FloorIssueReceive/GetFloorIssueDetailsItemByIssueId/" + issueId;
        AjaxManager.GetJsonResult(serviceUrl, jsonParam, false, false, onSuccess, onFailed);
        function onSuccess(jsonData) {
            objItem = jsonData;
        }
...
var IssueReceiveItemListHelper = {
    InitIssueReceiveItemList: function () {
        IssueReceiveItemListHelper.GenerateItemGrid();
    },
    GenerateItemGrid: function () {
        $("#grdFloorIssueReceiveItemSummary").kendoGrid({
            noRecords: true,
            messages: {
            ...
            dataBound: function (e) {
                ...
                var issueReceiveQtyIndex = e.sender.wrapper.find(".k-grid-header [data-field=" + "ReceiveQty" + "]").index();
                var remarksIndex = e.sender.wrapper.find(".k-grid-header [data-field=" + "Remarks" + "]").index();
                rows.each(function (index, row) {
                    var dataItem = e.sender.dataItem(row);
                    var tableHeadRow = $("#grdFloorIssueReceiveItemSummary thead tr");
                    if (!dataItem.name) {
                        tableHeadRow.find('th:eq(' + issueReceiveQtyIndex + ')').addClass('editable');
                        tableHeadRow.find('th:eq(' + remarksIndex + ')').addClass('editable');
                    } else {
                        tableHeadRow.find('th:eq(' + issueReceiveQtyIndex + ')').addClass('non-editable');
    ...
    LoadIssueItemListGridByIssueId: function (obj) {
        if (obj !== null) {
            ...
            var grid = $("#grdFloorIssueReceiveItemSummary").data("kendoGrid");
            grid.dataSource.data([]);
            var itemList = IssueReceiveItemListManager.GetIssueDetailsItemByIssueId(obj.IssueId);
